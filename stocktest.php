...
.buy-button {
  ...
  <body>
    <div class="grid-container">
      ...
      <a href="stockbuyorder.php" class="btn btn-success">สั่งซื้อสินค้า</a>
        ...
        <hr><br>
      <?php 
      $sql = "SELECT tra_name, tra_amount from transection " ;
  ...
  <tbody>
  <?php 
  $sqll = "SELECT * FROM tbl_stock";
  $resultt = mysqli_query($connect,$sqll);

while($row = mysqli_fetch_assoc($result)) {
    echo "<tr>";
    echo "<th>" . $order++ . "</th>";
    echo "<td>" . strtoupper($row['stock_name']) . "</td>";
    
    if ($row['tra_amount'] % 1 === 0) {
      $sqlCoffee = "UPDATE tbl_stock SET stock_amount = stock_amount - 1 WHERE stock_name = 'ผงกาแฟ'";
      $sqlMilk = "UPDATE tbl_stock SET stock_amount = stock_amount - 1 WHERE stock_name = 'นม'";
      $resultupdate = mysqli_query($connect, $sqlCoffee);
      $resultupdatemike = mysqli_query($connect, $sqlMilk);
  }
